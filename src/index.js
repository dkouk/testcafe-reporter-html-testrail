...
TestRail.prototype.apiPost = function (
    apiMethod,
    body,
    queryVariables,
    callback
...
TestRail.prototype._callAPI = function (
    method,
    url,
    queryVariables,
    body,
...
TestRail.prototype.updateConfigGroup = function (
    config_group_id,
    data,
    callback
) {
...
TestRail.prototype.updatePlanEntry = function (
    plan_id,
    entry_id,
    data,
    callback
...
TestRail.prototype.getResultsForCase = function (
    run_id,
    case_id,
    filters,
    callback
...
TestRail.prototype.addResultForCase = function (
    run_id,
    case_id,
    data,
    callback
    ...
    return {
        ...
        reportTaskStart(startTime, userAgents, testCount) {
            ...
            if (this.EnableTestrail) {
                if (
                    ...
                    this.newline().write(
                        this.chalk.red.bold(
                            "Error:  TESTRAIL_HOST, TESTRAIL_USER, TESTRAIL_PASS and PROJECT_NAME must be set as environment variables for the reporter plugin to push the result to the Testrail"
                        )
                    );
                    process.exit(1);
                }
        ...
        publishResultToTestrail() {
            ...
            for (var index in this.testResult) {
                console.log({ index });
                console.log(this.testResult[index]);
        ...
        generateReport() {
            ...
            for (var index in this.testResult) {
                ...
                if (status === "Skipped")
                    status = `<td style='background-color:gray' >Skipped</td>`;
                else if (status === "Passed")
                    status = `<td style='background-color:green' >Passed</td>`;
            ...
            var dir =
                process.env.HTML_REPORT_PATH ||
                `${__dirname}../../../../TestResult`;
